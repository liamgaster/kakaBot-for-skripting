...
# Place this at the very top of your entrypoint (app.py) BEFORE any other imports.
import importlib.util, pkgutil, types

if not hasattr(pkgutil, "get_loader"):
    def _get_loader(name_or_module):
        try:
            # Accept either a module object or a module name string
...
def dashboard():
    ...
    if request.method == "POST":
        # Save settings and/or start engine
        ...
        engine.load_script(script_text)@app.route("/dashboard", methods=["GET","POST"])
def dashboard():
    if "user" not in session:
        return redirect(url_for("home"))
    global engine, engine_thread
...
# Simple sample DSL to show in editor
...
if __name__ == "__main__":
    # Get port assigned by Railway, defaulting to 5000 for local testing
